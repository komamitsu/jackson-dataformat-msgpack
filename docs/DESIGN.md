...
could not be repaired afterwards because the bytes are already gone. So `flush()` returns
without doing anything unless the write context is back at root, and the writer asserts that
the decision was right:

```mermaid
flowchart TD
    F["flush()"] --> R{"writeContext.inRoot()?"}
    R -- "no: containers open" --> S["return: headers still to patch"]
...
A map key must be a scalar. `checkKeyWritable` enforces that before the name is recorded or
any byte is written, and `packKey` then writes the value natively: a string, an integer, a
float, a boolean, nil, binary or an extension. `MessagePackKeySerializer` exists for exactly
this, letting a `Map<Integer, V>` keep integer keys on the wire.

Anything else is refused, because no Jackson property name can represent it: the parser
rejects a map or an array in key position, so writing one would produce data this library
cannot read back, and Python and Go cannot represent such a key at all. A key of another type
needs a key serializer that writes a *name* rather than a value.

Serializing `Map<Integer, String>` holding `42` to `"ok"` is therefore flat, with the key
written straight into the parent's buffer between the reserved header and the value:
...
- `MessagePackGeneratorTest` / `MessagePackParserTest`: the Jackson layer, including close
  semantics, contexts, map keys and the accessor contract.
